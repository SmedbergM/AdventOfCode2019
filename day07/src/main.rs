...
fn amp_stack_feeback(program: &Program, perm: &Vec<u8>) -> Option<i64> {
    ...
    for i in std::iter::repeat(0..5).flatten() {
        ...
        let next_state = amp.await_output();
        if let State::Output(out) | State::OutputAwaitingInput(out) = next_state {
            last_output = out;
        } else {
